#!/usr/bin/env python
# coding=utf-8
"""ARM NEON BF16 MoE accuracy tests for KT-Kernel.

Tests accuracy of the ARM NEON BF16 MoE kernel (NEONBF16_MOE) against a
PyTorch reference, covering both decode (qlen=1) and prefill (qlen>1) paths.
Only runs on aarch64/arm64 hosts where the NEON kernel is compiled in.
"""
...
# Add test/ to path for ci registration
...
_IS_ARM64 = _HOST_ARCH in ("aarch64", "arm64")

if not _IS_ARM64:
    pytest.skip("ARM NEON accuracy test requires an aarch64/arm64 host", allow_module_level=True)
...
except Exception as exc:  # pragma: no cover - exercised on an ARM build failure
    raise RuntimeError("kt_kernel failed to import on an ARM64 host") from exc

pytestmark = pytest.mark.cpu

if NEON_MOE_CLS is None:
    raise RuntimeError("NEONBF16_MOE is missing from an ARM64 kt-kernel build")
...
@pytest.mark.parametrize("qlen,label", [(1, "Decode"), (16, "Prefill"), (33, "PrefillOdd")])
def test_neon_bf16_accuracy(qlen, label):
    """Test ARM NEON BF16 MoE accuracy (covers aligned and odd q lengths)."""
    ...
    with torch.inference_mode():
        ...
        for i in range(validation_iter):
            ...
            # NEON accumulation order differs from AVX2/AMX slightly; BF16
            # rounding noise should stay well below 2% on average.
            assert diff < 0.02, f"NEON BF16 accuracy test failed: diff={diff:.6f} >= 0.02"
...
if __name__ == "__main__":
    print("=" * 60)
    print("ARM NEON BF16 MoE Accuracy Test")
    print("=" * 60)

    try:
        test_neon_bf16_accuracy(qlen=1, label="Decode")
        test_neon_bf16_accuracy(qlen=16, label="Prefill")
        test_neon_bf16_accuracy(qlen=33, label="PrefillOdd")
