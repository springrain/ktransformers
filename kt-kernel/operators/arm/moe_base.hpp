/**
 * @Description  : ARM NEON MoE base class (ported from avx2/moe_base.hpp)
 * @Author       : Claude
 * @Date         : 2026-08-09
 * @Version      : 1.0.0
 * @Copyright (c) 2024 by KVCache.AI, All Rights Reserved.
 *
 * All AVX2 intrinsics (__m256, _mm256_*) replaced with NEON via the v8f32
 * helpers in arm/neon_bf16_utils.hpp (two 128-bit registers per 8 lanes).
 * Tile configuration calls (T::config()) are kept but are no-ops.
 **/
#ifndef CPUINFER_OPERATOR_ARM_NEON_MOE_BASE_H
#define CPUINFER_OPERATOR_ARM_NEON_MOE_BASE_H
...
class NEON_MOE_BASE {
 public:
  ...
  void forward_prefill(int qlen, int k, const int64_t* expert_ids, const float* weights, const void* input,
    ...
    // Pack input into BufferA (trivial memcpy for NEON)
    ...
    if constexpr (has_fused_prefill()) {
      ...
      // Activation: SiLU(gate) * up — NEON version (8 elements at a time)
    ...
    // Weighted sum of expert outputs — NEON version (16 BF16 = 2x8 FP32 at a time)
    pool->do_work_stealing_job(
        qlen, nullptr,
        [this, output, k, expert_ids, weights](int i) {
          for (int e = 0; e < config_.hidden_size; e += 16) {
            ...
            armneon::store_v8f32(f32out, x0);
            armneon::store_v8f32(f32out + 8, x1);
          }
        },
        nullptr);
  }
...
// ============================================================================
// TP_MOE specialization for NEON_MOE_BASE derived classes
// ============================================================================

template <class T, class Derived>
class TP_MOE<NEON_MOE_BASE<T, Derived>> : public TP_MOE_Common<NEON_MOE_BASE<T, Derived>, Derived> {
 public:
  using TP_MOE_Common<NEON_MOE_BASE<T, Derived>, Derived>::TP_MOE_Common;
  ...
  void merge_results(int qlen, void* output, bool incremental) override {
    ...
    auto merge_fn = [this, output, incremental, &config, &tp_count, &local_output_numa, &tp_configs](int token_nth) {
      ...
      if (incremental) {
        // Convert BF16 output to FP32 and add — NEON (16 BF16 at a time)
        for (int e = 0; e < config.hidden_size; e += 16) {
          armneon::v8f32 x0, x1;
          armneon::load_16xbf16_to_2x8xfp32((ggml_bf16_t*)output + token_nth * config.hidden_size + e, &x0, &x1);
          armneon::store_v8f32(merge_to + e, armneon::add_v8f32(armneon::load_v8f32(merge_to + e), x0));
          armneon::store_v8f32(merge_to + e + 8, armneon::add_v8f32(armneon::load_v8f32(merge_to + e + 8), x1));
        }
      }
      // Sum across TP parts
      for (int i = 1; i < tp_count; i++) {
        ...
        for (int e = 0; e < tp_configs[i].hidden_size; e += 8) {
          armneon::store_v8f32(merge_to + e,
                               armneon::add_v8f32(armneon::load_v8f32(merge_to + e), armneon::load_v8f32(merge_from + e)));
        }
      }
      // Convert FP32 -> BF16 output
      for (int e = 0; e < config.hidden_size; e += 16) {
        armneon::v8f32 x0 = armneon::load_v8f32(merge_to + e);
        armneon::v8f32 x1 = armneon::load_v8f32(merge_to + e + 8);
        armneon::store_2x8xfp32_to_16xbf16(&x0, &x1, (ggml_bf16_t*)output + token_nth * config.hidden_size + e);
      }
    };
...
#endif  // CPUINFER_OPERATOR_ARM_NEON_MOE_BASE_H
