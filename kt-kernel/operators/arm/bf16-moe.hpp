/**
 * @Description  : ARM NEON BF16 MoE operator (ported from avx2/bf16-moe.hpp)
 * @Author       : Claude
 * @Date         : 2026-08-09
 * @Version      : 1.0.0
 * @Copyright (c) 2024 by KVCache.AI, All Rights Reserved.
 *
 * Inherits the AVX2 simplifications over the AMX version:
 ...
 * Compute goes through armneon::gemm_bf16 (BFDOT when available, FMA otherwise)
 **/
#ifndef CPUINFER_OPERATOR_ARM_NEON_BF16_MOE_H
#define CPUINFER_OPERATOR_ARM_NEON_BF16_MOE_H

#include "neon_bf16_gemm.hpp"
#include "moe_base.hpp"

template <class T = armneon::GemmKernelNeonBF16>
class NEON_BF16_MOE_TP : public NEON_MOE_BASE<T, NEON_BF16_MOE_TP<T>> {
  using Base = NEON_MOE_BASE<T, NEON_BF16_MOE_TP<T>>;
  using Base::config_;
  using Base::down_ba_;
  using Base::down_bb_;
  using Base::down_bc_;
 ...
 public:
  ...
  NEON_BF16_MOE_TP() = default;

  NEON_BF16_MOE_TP(GeneralMOEConfig config, int tp_part_idx_ = 0) : Base(config, tp_part_idx_) {}

  void derived_init() {
    printf("Created NEON_BF16_MOE_TP %d at numa %d\n", tp_part_idx, numa_node_of_cpu(sched_getcpu()));
#if defined(__ARM_FEATURE_BF16_VECTOR_ARITHMETIC)
    printf("NEON BF16 path: native BFDOT instructions\n");
#else
    printf("NEON BF16 path: FP32 FMA fallback (no BFDOT on this build target)\n");
#endif
  }
  ...
  // GEMM dispatch — uses armneon::gemm_bf16
  void do_gate_up_gemm(bool do_up, int expert_idx, int ith, int nth, int qlen) {
    ...
    armneon::gemm_bf16(m, config_.intermediate_size, config_.hidden_size, *ba, *bb, *bc, ith, nth);
  }

  void do_down_gemm(int expert_idx, int ith, int nth, int qlen) {
    int m = m_local_num_[expert_idx];
    armneon::gemm_bf16(m, config_.hidden_size, config_.intermediate_size,
...
// ============================================================================
// TP_MOE specialization for NEON_BF16_MOE_TP
// Handles per-expert pointer loading and TP weight splitting
// (Ported from amx/bf16-moe.hpp TP_MOE<AMX_BF16_MOE_TP<K>>)
// ============================================================================
template <typename K>
class TP_MOE<NEON_BF16_MOE_TP<K>> : public TP_MOE<NEON_MOE_BASE<K, NEON_BF16_MOE_TP<K>>> {
 public:
  using Base = TP_MOE<NEON_MOE_BASE<K, NEON_BF16_MOE_TP<K>>>;
  using Base::Base;
...
#endif  // CPUINFER_OPERATOR_ARM_NEON_BF16_MOE_H
