...
class TP_MOE<AMX_FP4_MOE_TP<K>> : public TP_MOE<AMX_MOE_BASE<K, AMX_FP4_MOE_TP<K>>> {
 public:
  ...
  void write_weight_scale_to_buffer_with_pool(
    ...
    if (writer_pool == nullptr) throw std::runtime_error("AMX FP4 staging: writer pool is null");
    ...
    if (map != nullptr) {
      ...
      if (physical_expert_id < 0)
        throw std::runtime_error("AMX FP4 staging: expert is absent from physical-to-logical map");
    }
