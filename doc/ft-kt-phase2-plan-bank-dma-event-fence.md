# Phase-2 计划:cpu-off-datapath(per-rank pinned bank)+ event-fence(E_chunk 环块)—— ✅ 代码落盘,静态验证全过;✅ 2026-09-17 参数化落盘(8 env → `--kt-*` CLI,两主开关默认开,§6-21)
...
> 行号均为参数化落盘后快照(kt_ep_wrapper.py 7516 行 / kt_bank_dma.py 394 行 / 测试 664+726 行),后续以符号锚点重定位;锚点漂移见 §6-15。

## 0. 定位

两项绑定改造,编辑面八件:

- `kt-kernel/bench/bench_fp4_moe.py`(步骤 A:pack-bank 打包器,golden 区落 `bank/tile_layer{L}_rank{R}.bin` + `bank/manifest.json`)
- `kt-kernel/cpu_backend/cpuinfer.h` + `kt-kernel/ext_bindings.cpp`(步骤 B:task_tag 最小 ABI)
- **新建** `third_party/sglang/python/sglang/srt/layers/moe/kt_bank_dma.py`(394 行)
- `third_party/sglang/python/sglang/srt/layers/moe/kt_ep_wrapper.py`(D 换源 dispatch + F 环块全链)
- **新建** `third_party/sglang/test/manual/kt_bank_dma_test.py`(D1–D9,664 行)
...
1. **cpu-off-datapath(D,步骤 C)**:消 rank0 代写(≈340GiB/chunk 的 TP0 双写环路)。每 rank 进程本地 pinned bank + pack 期预切片,装载期按 rank 切片直读,TP0 提交段整段跳过,H2D 换源 bank 行。按裁定**pretiled 前置降级自带**(manifest sha256↔有效性位图、容量/NUMA 硬预检↔RAM 门槛、热更条款落文档;tier-5 pretiled-banks 本体维持🔒暂锁)。
...
## 1. 参数总表(8 枚;全归 ServerArgs `--kt-*`,wrapper 直读 `get_exec().moe`,environ.py 零 diff——无新增 env)
...
| `--kt-direct-bank-dma 0|1`(`kt_direct_bank_dma`,int 注解推导 type + `Arg(choices=[0,1])`) | {0,1} | **1** | `_negotiate_bank_dma_flag` `:1519` 本地读袋 + MIN 冻结;bank manifest 发现 + 校验;**默认开而无 manifest 时恰一行 warn(reason literal)强制落 0 = 预期常态**(§7) | per-rank pinned bank 开关 |
| `--kt-dump-slot-bytes 0|1`(`kt_dump_slot_bytes`,int 注解推导 type + `Arg(choices=[0,1])`) | {0,1} | 0 | DUMP 钩子直读袋(`:4691` 类) | readback 前必须 `raw_ready_event.synchronize`(D9) |
| `--kt-bank-dma-batch 0|1`(`kt_bank_dma_batch`,int 注解推导 type + `Arg(choices=[0,1])`) | {0,1} | 0 | **reserved**:server_args 登记,一期不消费不 warn | 二期删 no-op 共识;本期 inert |
| `--kt-bank-dma-lean 0|1`(`kt_bank_dma_lean`,int 注解推导 type + `Arg(choices=[0,1])`) | {0,1} | 0 | 同上 reserved 登记 | 二期;本期 inert |

**互斥矩阵**:BANK×FENCE **正交双开**(ring 块 H2D 源 = bank 专家行,TP0 提交跳过);**FENCE×WINDOW≥1 ⇒ FENCE inert + warn-once**(mutex,staging window 本期拥有 host 传输);**BANK×WINDOW≥1 ⇒ 一期 BANK inert + warn**;FENCE 不接 INT4/FP8/BF16(仅 MXFP4);BANK 仅 MXFP4 layerwise/legacy。
...
- **`--kt-direct-bank-dma 0|1`(默认 1)**——**每 rank pinned 权重仓总闸**。init 时发现 `<weight_path>/bank/manifest.json`(fail-closed 四组 key 校验,D1/D1b);进仓后专家行直接作 H2D 源,绕过 legacy 双槽 SHM staging;校验/容量任一失败强制落 0,恰一行 [KT-DEGRADE] warn,legacy 路径逐位等价。init 期 all-rank MIN 冻结(restart-required);**默认开而无 manifest 时每启动恰一行 `manifest_unreadable` warn 强制落 0 = 预期常态**(§7 baseline 口径)。与 window≥1 互斥(inert + warn);仅 MXFP4。
...
- **`--kt-dump-slot-bytes 0|1`(默认 0)**——**槽位字节级取证器**。开时每个装载槽在 raw-ready fence 之后 dump raw 字段字节,readback 前恒先 `raw_ready_event.synchronize()`(D9);文件落 `./kt_slot_dump/`。供字节级 parity/撕裂取证,不改装载行为。
- **`--kt-bank-dma-batch 0|1`(默认 0)**——**二期批量 bank-DMA 合成的户口**。Reserved:本期 inert、不消费不 warn,登记占名防复用;二期评估整窗单条/批量异步合并。
- **`--kt-bank-dma-lean 0|1`(默认 0)**——**二期 no-op 共识裁剪的户口**。同上 Reserved;二期评估削掉恒空共识往返。

> 消费点速查:bank `_negotiate_bank_dma_flag` 本地读袋 + MIN 冻结;fence `_event_fence_env_enabled`(直读袋)→ intent MIN;chunk `_event_fence_chunk_env` 折叠语义;fence-debug `_fence_debug_env_local` → `_any_tp_rank_true` MAX 冻结;dump 钩子 `:4691` 直读袋;no-device-sync 本期零消费(prepare 单读校验);两 reserved 仅 server_args 登记。

## 2. 架构

### 2.1 D:per-rank pinned bank

- 每 rank 进程本地(**非 shm**)pinned 竞技场,**永不 free**;落盘采用 `torch.empty(pin_memory=True)` 逐字段分配 + copy + view,半建失败由 **GC 回收半建 bank**(cudaHostAlloc 经 pin_memory 路径无需 unregister 簿记,R18;实现与计划模板的分歧见 §6-17)。
- init 流水逐步 fail-closed:①`_bank_manifest_validate` 纯函数四组键(topo/dims/dtype/map sha256 + weights hash + layout_version=1 + tile 表 field/rows/nbytes/sha256)→ ②`_bank_capacity_precheck`(复用 `_staging_capacity_sources` 类三源,RAM/memlock/NUMA 腿独立,先于任何大块分配;NUMA 腿为**各节点空闲之和**的 sum 语义,见 §6-18)→ ③MIN 冻结 + 校验串 all_gather_object 分叉 → 同消息 raise → ④逐层 fill(pageable 读 → pinned逐字段 copy + view;峰值 2×层字节,C++ 零新增)。任一步失败恰一行 warn(reason literal)+ 字段保 None = 回退面。
- 切片代数 `_bank_w13_offset/_bank_w2_offset`:W13 [gate;up] global-N 行切、**ue8m0→bf16 加宽在打包写盘时完成**;W2 global-K 边界;打包器写入侧与装载读取侧双源互核(D3)。
- 换源:legacy raw 环路 + planner 源 → bank 行(16B 对齐硬断言先于一切 copy;coverage 守卫位置不动);TP0 `_submit_host_write` 整段跳过;双槽环路两点共识保留 no-op error_broadcast;bank 覆盖全部专家含 GPU 常驻段、**行只读**、热更段零 diff。
...
- **共识计数精确口径**:legacy = 2×E;FENCE=1 ⇒ 2×⌈E/E_chunk⌉;window(w1/w2)= 2/层;BANK 一期不增减(no-op error_broadcast 保留)。集合通信运行时**零新增**。
...
`bench_fp4_moe.py` golden 区扩 pack-bank:按 rank 切片写盘 `bank/tile_layer{L}_rank{R}.bin` + `bank/manifest.json`(四组键 + weights hash + `layout_version=1` + tile 表 field/rows/nbytes/sha256,`sort_keys=True` 确定性)。生产权重实跑与耗时/RAM 峰值入库为尾项②(tier-5 条件⑤回执)。

### 步骤 B(C++ 最小 ABI)

`cpuinfer.h`:worker 异常将 `task_tag` 解 (kind, expert_id) 前缀进 `what()`;Args 默认 0,热路径零成本。`ext_bindings.cpp`:Args 尾部追加参数(双 submit 重载合一、inner submit 专用 `delete args_` 路径同步)。旧 binding ⇒ D8 raise "Rebuild kt-kernel"(探针 + 闩锁 + skip 阀门);**aarch64 重编译登记尾项①**。

### 步骤 C(D wrapper 侧)

新建 `kt_bank_dma.py`(394 行):msgspec `_BankManifest/_BankTileEntry/_BankGeometry/_RankBank` + 纯函数族(校验、预检、切片代数、行视图)。wrapper:八枚开关全归 CLI 参数,直读 `get_exec().moe`(batch/lean 两枚 reserved 仅 server_args 登记、不消费不 warn,原 kt_bank_dma 模块级登记随参数化整体删除,模块回纯函数零 env,见 §6-21);`_bank_dma` **顶层导入**(正名见 §6-17:msgspec/torch 本就加载、无循环导入与 CUDA init 风险,参数关额外零成本无需懒导入);`initialize_cpu_buffers()` 挂 `_init_rank_bank()`(校验→预检→MIN+校验串冻结→分层 fill;失败恰一行 + 字段保 None);换源 dispatch(bank 分支 legacy 环路换源 + planner src→bank 行,16B 断言先,coverage 不动);TP0 提交段跳过;两共识 no-op;DUMP 钩子(raw_ready fence 同步先于 readback)。
...
## 4. 测试映射(D1–D9 / F1–F8;CPU stub,无 GPU 无 dist)
...
| # | 族 | 断言对象 |
|---|---|---|
| D1 | 关键路径簿记 | manifest 四组键各失配 → 各自 reason literal;D1b tile loader sha256/不可读/缺失三路 |
| D2 | 派生性质 | RAM/memlock/NUMA 腿独立门控;阈值 (50,80)+(51,80) 边界;探测失败 fail-closed(裁定 §6-2) |
| D3 | 派生性质 | 切片代数金色元组与 cpu_k/gpu_k 互证;16B 非对齐 raise |
| D4 | 关键路径簿记 | 参数关冻结 off 静默(零 warn);**默认开**下 missing-manifest(`manifest_unreadable`)/mutex(`mutex_window`)/missing-path(`no_weight_path`)三路恰一行 [KT-DEGRADE] 降级(默认开常态降级扶正为正测,§6-21) |
| D5 | 关键路径簿记 | 换源:16B 断言先、coverage 先、bank vs shm 行集并集逐位等、op 仅 src 差;row 视图共享存储;planner 契约不变 |
| D6 | 关键路径簿记 | 校验串分叉双 manager → 同消息 raise;peer error 降级一次 |
| D7 | — | (随 env→CLI 迁移**删除**:冻结 CLI 参数无运行期漂移通道可供检查观测,§6-21;编号跳档)abort/重复 init 张量 id 不变与半建失败回收路径仍登记实机/后续测试尾项(本批未落单测,计划口径见 §6-17) |
| D8 | 关键路径簿记 | task_tag ABI 探针:旧 doc raise "Rebuild kt-kernel"/新 doc pass + 闩锁 + skip 阀门 |
| D9 | 关键路径簿记 | DUMP sync 严格先于 readback 恰一次;参数默认关零副作用(原非法值 warn 链随 argparse 删除) |
...
2. D1–D9 / F1–F8 / N1–N11 / N1–N5 / N4–N6 五套件一次复跑全绿。
3. 魔数:本批三文件(wrapper + 两测试)全部新增行 `grep \b(256|24|41|61)\b` 零命中。
...
7. C++ 两文件 diff 逐行审(task_tag 热路径零成本);bench packer diff 逐行审。
8. 本文档 + tiers 总表回写(梯队 2 行 / cpu-off-datapath / event-fence / pretiled-banks 前置条件进度)。
9. **审查修复复核(同日 §6-17~20 落地后)**:py_compile 全触件通过;D1–D9 / F1–F8 / N1–N11 / N1–N5 / N4–N6 五套件一次复跑全绿(含 D2 sum 语义新边界、D4 reserved/baseline、F8 折叠/mutex 基线断言);新增行魔数扫描零命中;environ.py 零 diff 维持。
10. **参数化复核(2026-09-17,§6-21 裁定落地后)**:py_compile 全触件通过(server_args / wrapper / kt_bank_dma / 两测试);五套件一次复跑全绿(D4 参数关静默 + 默认开三路降级扶正;F6/D7 随迁移删除、编号跳档);8 名 env(`SGLANG_KT_{PREFILL_EVENT_FENCE,PREFILL_STAGE_CHUNK_EXPERTS,PREFILL_NO_DEVICE_SYNC,PREFILL_FENCE_DEBUG,DIRECT_BANK_DMA,DUMP_SLOT_BYTES,BANK_DMA_BATCH,BANK_DMA_LEAN}`)全仓 + 主仓 bench grep 零命中(仅历史叙述段除外:本文 §6-19 与 migration-audit 两文档,均已标注为历史);新增行魔数 `\b(256|24|41|61)\b` 扫描零命中;environ.py 零 diff 维持。

## 6. 裁定记录(施工期逐条;含计划↔落盘锚点漂移)

1. **批内序与范围**:用户裁定 D+F 同批一次性;event-fence 按先例现在施工(默认关=逐位等价,收益 TBD,归因门+门槛登记尾项);pretiled 前置降级自带,tier-5 本体维持🔒。
2. **d2 容量阈值边界 (75,80)→(50,80)+(51,80)**:预检为 ≤/不足判定,期望值修正为两源各自独立短缺两条(D2 双用例)。**#18 裁定起 NUMA 腿改 sum 语义**,该边界期望值由 §6-18 重述。
3. **D8/N5 ABI 探针同族**:`kt_degrade_reason_test.py` N5 与 D8 复用同一探针族(旧 doc raise/新 doc pass/闩锁/skip 阀门),文案 settle。
4. **weights 全量 hash 启动期不可行**:改 pack-time 算 hash + 启动廉价键(dims/map sha256/tile 表)+ DUMP 金丝雀;偏离逐字公示于此。
5. **RUN 冻结失败语义**:bank 校验失败/容量不足/NUMA 不符/协商分叉 → 恰一行 reason literal warn 强制落 0,不全 rank 崩(仅校验串 raise 属共识语义)。
...
9. **`_submit_host_write` 闭包 `numel() // 2` 不改**:INT4 packed 专属口径;ring 永不调它;bank 跳过 TP0 提交段。运行时原语零触碰。
...
11. **BATCH_DMA × ring**:ring 路径忽略 BATCH_DMA(ring 不建 planner、不经 `_enqueue_planned_h2d`);BANK×FENCE 双开时 `src_row = expert_id`(bank 行)。
...
17. **GC 回收模板 + 顶层导入正名**(审查修复 #1/#2):计划原写的「半建失败 drain transfer_stream → unregister → free」模板与落盘实现不符——落盘为 `torch.empty(pin_memory=True)` 逐字段分配 + copy + view,半建失败置 `tensors={}` + `gc.collect()` 后 raise,cudaHostAlloc 经 pin_memory 路径无 unregister 簿记,GC 即完整回收(R18 语义不变:bank 永不 free,半建品也不例外)。capability 侧 `_bank_dma` 为**顶层导入**而非懒导入:msgspec/torch 本就由 wrapper 加载,顶层导入无循环导入与 CUDA init 风险,unset 同样零成本。文档 §2.1/§3-C/§4-D7/§10/R18 行同步改写;D7 原宣称的「abort/重复 init 张量 id 不变;半建失败调用序」两条经全文件 grep 坐实**测试未落**,表行改为实机/后续测试尾项。
18. **NUMA 腿改 sum 语义**(审查修复 #4):`_bank_capacity_precheck` NUMA 腿由「均分假设 per_node=⌈total/len⌉、任一节点 <per_node 即拒」改为「**各节点空闲之和 < total 即拒**」。理由:①Linux 默认 NUMA 策略页随首次触分配、本地节点不足时跨节点 fallback,真实容量约束是聚合空闲而非均摊假设——落盘的逐字段 fill 并无均分实现背书,均分判据会误杀 (free0=0, free1≥total) 这类完全可服务布局;②「任一节点 ≥ total」反向判据则过松;③单节点 len=1 时 sum ≡ total,精确退化。D2 期望同步改写:(50,80)/(51,80) 现为 True,新增 (20,80) False 边界与判别用例 (0,200) True;§6-2 阈值边界由本条重述。容量/NUMA 实测仍登记尾项⑦。
19. **reserved env 模块级登记补齐**(审查修复 #5):治理段所列 `SGLANG_KT_BANK_DMA_BATCH/LEAN` 原仅定义 `_kt_bank_reserved_env` 助手而**零调用点**,非法值永不 warn,「治理段登记」未兑现。修复:kt_bank_dma.py 模块级在助手定义后直接自调用两枚(import wrapper 即触发),非法值恰一行 warn 按 0;D4 补 bogus 恰一行 + latch 幂等断言。(**历史叙述**:env 时代的审查修复;§6-21 参数化后两枚 reserved 归 server_args 登记、不消费不 warn,上述助手/登记段/latch 整体删除)
...
- 组合矩阵逐格:baseline(`--kt-direct-bank-dma 0 --kt-prefill-event-fence 0` 双显式置 0 = legacy 逐字路径)vs `{BANK}`(仅 fence 显式置 0)/ `{FENCE}`(仅 bank 显式置 0)/ `{BANK+FENCE}`(**= 默认开档,零参数**)/ `{FENCE+NO_DEVICE_SYNC}` / `{FENCE}×WINDOW{0,1,2}`;每格 dump KT 层 MoE 输出 + 最终 logits,sha256 逐位相等,**容许差 = 0** + 每层 device 共识计数断言(= 2×⌈E/E_chunk⌉)。BANK 默认开而**无打包 manifest** 时恰一行 `manifest_unreadable` 降级 warn 强制落 0 = 预期常态,不算失败格(§6-21)。
...
| # | 红线 | 状态 |
|---|---|---|
| R1 | **默认开;显式置 0 opt-out 时逐位等价**(§6-21 语义翻转) | ✅ F4(参数关恰一次 intent MIN、geometry None、零 warn);D4(参数关冻结 off 静默,op 与 legacy 逐行等) |
| R2 | 参数归 ServerArgs `--kt-*`;environ.py 零 diff(无新增 env) | ✅ §5-4/§5-10 |
| R3 | 几何/开关协商期 MIN/MAX 冻结;CLI 参数运行期不可变,drift 机制废止(§6-20 注记/§6-21) | ✅ F4/F5/F7/D4 冻结面;drift 用例随机制删除 |
| R4 | 代码/测试零生产拓扑魔数 | ✅ §5-3 |
| R5 | 集合点 rank 对称零分叉;TP0-only 仅提交段 | ✅ F3 tp1 spine、D5 |
| R6 | 共识计数钉死;集合通信运行时零新增 | ✅ F2(2×⌈E/E_chunk⌉、chunk=1 ≡ 2×E);BANK no-op 保留 |
...
| R12 | begin-fence 三键 assert(传输不与热更新窗重叠) | ✅ F 块首 begin-fence;bank 行只读 + 热更段零 diff |
| R13 | msgspec / None-init / LOC / 注释风格 | ✅ §5-6 |
| R14 | bank 行只读、热更零 diff | ✅ D5 row 视图只读共享存储 |
| R15 | 预检 fail-closed 先于分配 | ✅ init 流水 ② 先于任何大块分配;D2 |
| R16 | shm 机构一期全留 | ✅ 不动 shm 分配/收集/unlink |
| R17 | 16B 对齐断言先、coverage 位置不动 | ✅ D3/D5;ring bank_nbytes 16B 硬断言 |
| R18 | bank 永不 free(半建失败 GC 回收;原 drain→unregister→free 模板按 §6-17 降为文档口径) | ✅ pin_memory+GC 实现审查裁定 |
| R19 | 异常打标最小 ABI;旧 binding ⇒ D8 raise | ✅ D8/N5 探针 |
...
1. **aarch64 重编译** + D8 确认旧 binding raise("Rebuild kt-kernel")/新 binding pass。
2. packer 跑**生产权重**:manifest 生成正确性 + 耗时/RAM 峰值入库(tier-5 条件⑤回执)。
3. **benchbw 归因门**:F 收益恒 TBD,控制面缺口证实才计入;§6-3 init 集合计数 +1～+3 实机复核入库。
4. §7 对拍矩阵逐格 sha256(容许差=0)+ 每层共识计数断言 + E_chunk 退火(1×48h → 16/32/64×24h)。
5. 性能门槛:transfer_stream 带宽 ≥17GB/s(<11 回落)+ 画像 busy >50% 门。
6. 双请求 overlap + **热更 ON** sha256 + 单 rank kill 注入 + NCCL 2-rank 30min 延迟注入 soak。
7. bank 容量/NUMA **实测回填** tier-5 条件③(RAM 门槛数值);pretiled-banks 复审输入。
8. 回滚演练:`--kt-direct-bank-dma 0 --kt-prefill-event-fence 0` 双显式置 0 重启即回 legacy 逐字路径;E_chunk 阶梯地板 1 基线复现(§7 口径)。
9. **Phase-2 登记**:WINDOW 行对接(fence×window、bank×window 二期解禁)、`--kt-bank-dma-batch/--kt-bank-dma-lean` 两枚 reserved 参数激活、`NO_BATCH_MEMCPY` 三期评估、dma_done 换代归 3 梯队 memop、BufferB 装载 gap(四禁条款下继续暂锁)。

## 10. 回滚

- CLI 参数启动冻结不支持热关闭;显式置 0 / 校验失败 / 容量不足 / 协商降级 → 全 rank 统一落 0,legacy 路径逐字保全(从未被裁掉任何一行)。
- `E_chunk=1` ≡ legacy 2×E 语义(F2),是最近回退档(choices 不含 1,由阶梯地板达成)。
- `_bank_dma` 顶层导入(参数关亦零成本,§6-17);half-built bank 由 GC 回收(pin_memory 半建张量无 unregister 簿记)。
...
- **二期清单**:BATCH/LEAN 激活、fence×window 与 bank×window 解禁、dma_done 换代、BufferB gap——均不在本批。
